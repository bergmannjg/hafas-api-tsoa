...
import { Stop, Location, Station, Journeys, Movement, TripWithRealtimeData, Departures, DurationsWithRealtimeData } from './hafas-client-types';
import { createClient } from 'hafas-client';
import { profile } from 'hafas-client/p/db/index.js';

@Route("hafas")
export class HafasController extends Controller {
  private clientFactory() { return createClient(profile, 'client'); }
  ...
  */
  ...
  public async gettrip(
    ...
    const client = this.clientFactory();
    if (client.trip) return await client.trip(id, { stopovers, polyline, subStops, entrances, remarks, language }) as TripWithRealtimeData; else throw ReferenceError("method not defined");
  }
  /**
  ...
  */
  ...
  public async getdepartures(
    ...
    const client = this.clientFactory();
    return await client.departures(station, { when, direction, duration, results, subStops, entrances, linesOfStops, remarks, stopovers, includeRelatedStations, language }) as Departures;
  }
  /**
  ...
  */
  ...
  public async getreachableFrom(
    ...
    const client = this.clientFactory();
    if (client.reachableFrom) return await client.reachableFrom({ type: "location", address, longitude, latitude, distance }, { maxTransfers, maxDuration, subStops, entrances }) as DurationsWithRealtimeData; else throw ReferenceError("method not defined");
  }
  /**
